...
**Plan is the terminal blueprint gate.** The blueprint reset points are the **Scope → Architecture → Plan** approvals — Plan is the terminal gate, and the reset offered there, before handing off to per-feature spec-driven-dev, is the highest-value one; its `RESET-CHECKPOINT:` advisory frames Plan as the highest-value blueprint reset point and still lists the spec-driven-dev hand-off files to re-read. Note that CFC (Cross-Feature Contract) drift is **not** part of `--run-state`; run the full `validate_blueprint.py blueprint/` to check Cross-Feature Contracts before relying on a reset at the Plan gate.
