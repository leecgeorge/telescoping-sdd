...
**The flag is a doctrine-classified fact, never a convenience skip.** `--decline-pending` records that the USER decided to skip the panel for this obligation: a user decision the agent SURFACES and the user makes, never a self-serve shortcut the agent reaches for to dodge a panel it judged "unnecessary". (project-blueprint needs no Phase-4 task-tick carve-out and has no `--task-tick`: marking a feature **Done** by flipping its `## Milestones` checkbox `- [ ] F<n>` <-> `- [x] F<n>` is **hash-neutral** — `content_for_hashing` neutralizes milestone-feature checkbox state — so a milestone tick never moves the PLAN `## Approval` hash and there is nothing to re-stamp; the carve-out an SDD task-tick needs has no analog to require here.) Using `--decline-pending` to wave through a substantive re-stamp IS the factual-edit shortcut wearing a flag — the exact failure mode the pending-review marker (`upstream-panel [0-9a-f]{8}`) and the changed-document reminder make auditable.
