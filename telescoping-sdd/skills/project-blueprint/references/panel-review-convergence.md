...
A single, purely-subtractive compression pass that runs once when the panel converges, as the loop's last act before validation. It is the **sole** subtractive actor over the prose the loop accreted across its passes — the per-pass `Addressed` disposition only adds, so the redundancy those passes introduced is compressed in one place, here, before the human reviews the artifact. (Drafting-time `bake-resolution` is *preventive* hygiene — it keeps the debate out of the artifact in the first place — not a subtractive actor over already-written prose.)
