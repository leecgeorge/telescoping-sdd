...
**Each approval gate is a safe context-reset checkpoint.** Once a phase is approved (and, for Phase 4, after the completion checklist), the orchestrator **offers a context reset** here — its decisions are durable on disk, so you can reset the context (`/clear`, `/compact`, or a fresh session) to cap window growth, then run `validate_spec.py <spec-dir> --run-state` to rehydrate current phase / approval / hash / obligation state in one read-only screen. The offer is gate-triggered and non-blocking — accept or decline it; it is never automatic or forced. First confirm the phase's `## Approval` box is ticked and your conversational decisions are written into the artifact (a reset drops un-written nuance; not safe mid-panel-loop). Full doctrine: `references/workflow-overview.md` § "Context Management — Resetting at Gates".
...
**Before the first task, state the resolved stack profile and what it means for TDD:** python/java → **TDD is mandatory** (test-first MUST) for every code-touching task; `generic`/architecture-neutral → **TDD teeth are OFF** (the Verification-check path applies). This is the same profile the validator's R3/R5 gates key on, so the practitioner sees up front whether the test-first MUST is active.

Execute tasks sequentially following this cycle for each task:

1. Read the task from tasks.md
2. **Establish the check first.** For a **code-touching task on a python/java stack this is a hard MUST, not advice:** you **MUST** write the failing test(s) that encode the task's acceptance criteria **before any production code**, run them, and **confirm they fail for the expected reason** (the **red** phase) — *then* implement to make them pass (**green**), *then* **refactor**. You **MUST NOT** write production code before its failing test exists. Skipping this test-first cycle for a legitimate reason is permitted but **MUST** be recorded in the `## TDD Exceptions` ledger (step 5) — it is an auditable act, never a silent omission. *(A code-touching task that genuinely needs **no** test at all is not a skip — declare `**Tests:** none — <reason>` in tasks.md; the R3 gate enforces that distinction.)* For a `generic`/architecture-neutral stack with no harness, the MUST does **not** apply: write down the task's concrete Verification check (the runnable assertion, manual step, or review step) before doing the work.
3. Do the work to satisfy the acceptance criteria — make the failing tests pass (**green**) and then **refactor**, or produce the config / page / infra / doc / skill artifact. **The task's acceptance criteria are the boundary:** satisfy them completely — including the refactor of the code this task touches — and stop there. Work beyond that boundary (a neighbouring fix, an unasked-for improvement, another task's work) is a discovery to log, not scope to absorb — see *Mid-implementation discovery* below. Completing a task fully and widening it are different things.
...
5. Update tasks.md immediately — do all of the following before moving to the next task:
   ...
   - **If (and only if) you skipped the test-first cycle** on a code-stack task, append one `pending` row to the `## TDD Exceptions` ledger recording the task ID and the skip reason (code stacks only; the append is carve-out-eligible — silent re-stamp, no Re-Approval trigger, per `references/hash-and-cascade.md`)

### Mid-implementation discovery

While implementing, if you deviate from the approved spec/design — or realise you must — because of something you learned, or find yourself doing work no task asks for, do **not** silently continue. This is **NOT** a task-tick edit; the Phase-4 task-tick carve-out does not apply. Triage the deviation by an observable test — it is **major** if it:

(i) changes or invalidates any acceptance criterion; (ii) changes any external interface/contract; (iii) adds or removes an external dependency; (iv) affects any remaining (not-yet-done) task — invalidates it, or changes a spec/design statement that task will read as ground truth; or (v) changes a security or privacy surface (e.g. how a secret/credential is handled or logged, an authentication/authorization check, an input-validation rule, or what data is exposed). **"When in doubt → major."** Only genuinely-local choices where all five are false stay **minor**. **Unrequested work is minor by default** — the when-in-doubt rule does **not** apply to it. Log it and continue; it becomes major only when it independently trips one of the five criteria above (most often (iv), when it does work a not-yet-done task owns). The point is an auditable record of what you did beyond the task, not a halt.

- **Major → backport now.** Halt, surface the deviation to the user, and edit the **single highest-affected upstream** document (`spec.md` if an acceptance criterion is affected, otherwise `design.md`). **Delegate that upstream edit to the analyst for the phase of the artifact being edited** — the *upstream* artifact's phase, not the current (Tasks) phase. Example: a discovery during Tasks that corrects an acceptance criterion in `spec.md` is drafted by `telescoping-sdd:feature-spec-analyst`, **not** `telescoping-sdd:feature-task-analyst`; a `design.md` correction goes to `telescoping-sdd:feature-architecture-analyst`, and a `tasks.md` correction to `telescoping-sdd:feature-task-analyst`. Supply the delegation brief and run the scope-verify + content-preservation check (`references/hash-and-cascade.md` § "Revise the downstream") before the edit runs *Re-Approval After Edits* (re-stamp → upstream panel re-review → cascade back down to `tasks.md`). Edit only that one document and let the cascade produce the downstream changes — never co-edit the chain to force consistency. Then resume.
...
- **Resolve the `## Implementation Deviations` ledger (completion gate).** If `tasks.md` carries a `## Implementation Deviations` section with `pending` rows, present each to the user with its target upstream document and the matched triage reason, and ask per entry whether to backport: **yes** → backport that entry (delegate the single highest-affected upstream edit to the analyst for that artifact's phase — per the phase-to-analyst mapping in "Major → backport now." above — supplying the delegation brief and running the scope-verify check per `references/hash-and-cascade.md` § "Revise the downstream"; → *Re-Approval After Edits* → mark the row `backported`); **no** → mark it `declined` and record a paired `## Accepted Divergences` entry **in `tasks.md`** (co-located with the ledger; lean-yes if the deviation touched an acceptance criterion or external contract, otherwise lean-no). Before re-stamping, verify every `declined` row has a paired `## Accepted Divergences` entry — a dangling decline halts the gate until drafted. Zero `pending` rows → the gate does not fire
- **Resolve the `## TDD Exceptions` ledger (completion gate).** If `tasks.md` carries a `## TDD Exceptions` section with `pending` rows (code stacks only — generic projects never have them), present **each** `pending` row to the user with its task ID and skip reason — **never auto-accept; review is mandatory for every `pending` row, even when the reason looks legitimate**. The user resolves each to one of the ledger's own two outcomes (NOT Deviations' `backported`/`declined`): **`accepted`** (the test-first *order* was skipped but the test now exists — the skip stands, recorded) or **`remediate`** (the test is missing → re-open the task into the Phase-4 loop; the row stays open until the test is written, at which point it transitions to `accepted`). Update each row's `Resolution` before re-stamping. A `Resolution` transition is **substantive** (not carve-out eligible). Zero `pending` rows (or no section) → the gate does not fire.
- **Surface the R4 override + cross-reference audit, then run the R5 existence check (code stacks).** Run `python <script-path>/validate_spec.py specs/F<n>-<slug>/ --completion-gate --project-root <repo-root>` — for a python/java stack this (a) re-surfaces the `tasks.md R4 test-exempt overrides` and `tasks.md cross-reference test delegations` audit counts so any test-exempt or delegated tasks are visible at the gate, and (b) verifies that every test name declared in a code-touching task's `**Tests:**` field **actually exists** in the codebase (R5). A declared-but-absent test is a **`[WARN]` by default** — surface it and let the practitioner decide (write the test, or accept it as a known boundary such as a renamed function); add `--strict-r5` to make a missing test a hard **`[FAIL]`** (exit 1) that blocks the gate. A `generic` stack returns a single skip-PASS — R5 does not apply. (Non-code tasks, R4-override tasks, and cross-reference tasks declare no by-name test of their own, so R5 does not check them.)
- **Re-stamp `tasks.md` once.** First run `python <script-path>/validate_spec.py specs/F<n>-<slug>/` and confirm structural validity (no `[TBD]`, no `TODO`/`FIXME` leaked into task descriptions, all required sections present, `## Panel Review` populated). If any structural check fails, halt and fix before re-stamping — re-stamping a structurally broken `tasks.md` would silently approve known-bad content. Then choose the re-stamp form by **inspecting the diff against the most recent `--approve tasks` stamp** — not the working tree: diff against the commit of the last `--approve tasks` (e.g. `git diff <last-approve-tasks-commit>..HEAD -- tasks.md`), so a substantive edit already committed earlier in the Phase-4 loop is not missed. (A substantive *mid-implementation* edit should already have triggered its own re-review per *Re-Approval After Edits*; this baseline guards the completion-gate diff specifically.)
  ...
  - **Substantive completion → plain `--approve tasks`.** If the completion gate also resolved either ledger — any `## Implementation Deviations` `Backport status` transition (`pending → backported`/`declined`), any `## TDD Exceptions` `Resolution` transition (`pending → accepted`/`remediate`), or a new `## Accepted Divergences` entry — or made any other out-of-section edit, that is substantive (per C2); run plain `python <script-path>/validate_spec.py specs/F<n>-<slug>/ --approve tasks` and resolve the resulting upstream-panel-re-review obligation per *Re-Approval After Edits*. (An already-logged `minor` `## Implementation Deviations` row or `pending` `## TDD Exceptions` row *appended during implementation* does not by itself make the completion substantive — that append was carve-out-eligible at append time; what makes the completion substantive is the gate's *resolution* of a ledger, above.)
