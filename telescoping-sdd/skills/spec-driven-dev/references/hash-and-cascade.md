...
2. **Then handle approval-state FAILs:**
   ...
   - **Phase 4 carve-out.** If the edit is a task-tick edit (per the task-tick discriminator above), re-stamp silently, skip the cascade, AND skip the upstream panel re-review step. All three actions are explicit — the new step's suppression is named directly here so a future text edit cannot accidentally drop it by re-ordering. Structural FAILs on `tasks.md` still halt at step 1. When the carve-out fires and suppresses the new step, emit a one-line debug note: `Upstream panel re-review: suppressed — Phase 4 task-tick carve-out active`.
     ...
     **Ledger-append exemption (Phase 4).** Appending a row to the `## Implementation Deviations` ledger in `tasks.md` (the minor-deviation path of the triage gate) is itself carve-out-eligible — treat it like a task-tick: silent, no Re-Approval trigger, sealed by the single Final-Check re-stamp. The exemption is narrow: it applies only when the diff is **append-only** AND all changed lines fall entirely within the `## Implementation Deviations` section AND every appended row's `Classification` is literally `minor`. Status transitions and out-of-section edits still trip the substantive path: any edit outside that section, any non-append edit, and any `Backport status` transition (`pending → backported` or `pending → declined`, performed at the completion gate, not the task loop) — so the carve-out cannot widen into a cascade-bypass hole. The deviation itself is substantive (NOT a task-tick); only the clerical act of recording an already-classified `minor` row is exempt — the triage gate (`SKILL.md` § "Mid-implementation discovery") is the sole classifier, and this append is mere transcription.
