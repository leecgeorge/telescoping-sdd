...
**Phase 4 (Implement) is the exception.** `tasks.md` gets ticked continuously during implementation; re-stamping after every tick would be noise. The Phase 4 cadence is: re-stamp once on resumption (handled by mid-stream entry), stay silent through every tick, then re-stamp once at completion (driven by the Final Check in SKILL.md — `--approve tasks --task-tick` for a completion whose diff since the last stamp is a confirmed pure-tick edit per the C2 discriminator, or plain `--approve tasks` if the completion also resolved the ledger). Substantive mid-implementation edits to `tasks.md` (re-scoping, adding tasks because design proved incomplete) still follow the full flow below.
...
> **This is a first-class, proactive context-management practice — not only a crash-recovery procedure.** `references/workflow-overview.md` § "Context Management — Resetting at Gates" tells you *when* it is safe to reset the context (at any phase-approval gate); this section is *how* you re-orient afterward. For a fast one-screen orientation, run `validate_spec.py <spec-dir> --run-state` (read-only — it re-derives current phase, per-artifact approved/hash status, and open obligations without changing anything). The full procedure below re-establishes and, where needed, repairs approval state in detail — use it whenever you enter an in-progress workflow, whether after a **deliberate reset at a gate**, an involuntary compaction, a `/clear`, a fresh session, or a `git pull`/`merge`/branch switch.
...
2. **Then handle approval-state FAILs:**
   ...
   - **Phase 4 carve-out.** If the edit is a task-tick edit (per the task-tick discriminator above), re-stamp silently, skip the cascade, AND skip the upstream panel re-review step. All three actions are explicit — the new step's suppression is named directly here so a future text edit cannot accidentally drop it by re-ordering. Structural FAILs on `tasks.md` still halt at step 1. When the carve-out fires and suppresses the new step, emit a one-line debug note: `Upstream panel re-review: suppressed — Phase 4 task-tick carve-out active`.
     ...
     **Ledger-append exemption (Phase 4).** Appending a row to a **recognized Phase-4 ledger section** of `tasks.md` — either `## Implementation Deviations` (the minor-deviation path of the triage gate) or `## TDD Exceptions` (a logged test-first skip) — is itself carve-out-eligible: treat it like a task-tick — silent, no Re-Approval trigger, sealed by the single Final-Check re-stamp. The exemption is a narrow **section-scoped conjunction**: it applies only when the diff is **append-only** AND all changed lines fall entirely within a **single** recognized Phase-4 ledger section (a straddle of both ledgers is NOT exempt) AND, for an `## Implementation Deviations` append, every appended row's `Classification` is literally `minor` AND, for a `## TDD Exceptions` append, every appended row's `Resolution` is literally `pending`. The `Classification` is literally `minor` conjunct is the SOLE guard keeping a *major* deviation out of the silent re-stamp path — the carve-out is doctrine-only (no validator inspects the diff), so the prose IS the guard; never drop it. Status transitions and out-of-section edits still trip the substantive path: any edit outside a single ledger section, any non-append edit, and any status transition — an `## Implementation Deviations` `Backport status` transition (`pending → backported` or `pending → declined`) or a `## TDD Exceptions` `Resolution` transition (`pending → accepted` or `pending → remediate`), each performed at the completion gate, not the task loop — so the carve-out cannot widen into a cascade-bypass hole. The deviation or skip itself is substantive (NOT a task-tick); only the clerical act of recording an already-classified `minor` Deviations row, or an initial `pending` TDD Exceptions row, is exempt — the triage gate (`SKILL.md` § "Mid-implementation discovery") and the TDD-skip log are the sole classifiers, and this append is mere transcription.
...
- **Revise the downstream** — Claude classifies the fix:
  ...
  - **Substantial** (new requirements, acceptance criteria, components, interface contracts, security/privacy surfaces, or external dependencies — same criteria as the panel re-review step below) — Claude describes the proposed approach in one or two sentences and asks explicitly: *"Proceed with this approach?"* On yes, the orchestrator **delegates the draft to the phase's analyst agent** (the "Drafted By" analyst for the artifact being revised) rather than authoring it directly, so a substantial out-of-loop revision carries the same authorship discipline as the original draft. **Delegation brief** — the invocation gives the analyst: (1) the specific concern/divergence to fix, scoped to that change only; (2) the originating rationale (the *why*), which for a cascade/backport/gate-change lives in downstream or conversation context the fresh analyst cannot otherwise see; (3) the user-approved approach (the *"Proceed with this approach?"* answer); (4) the current `### Sealed dispositions` / `### Deferred dispositions` context to preserve verbatim; (5) an instruction to change nothing outside the intended edit and to preserve the `## Panel Review` and `## Approval` sections verbatim; (6) a **scratch output path** to `Write` the revised whole file to — never the live artifact; and (7) an instruction to *surface* genuine ambiguity (two reasonable interpretations, missing context, or a change that would exceed the approved scope) in its return manifest rather than silently resolving it. **Scope-verify before promote** — the analyst is a whole-file `Write` author, so the orchestrator checks the returned scratch draft before it touches the live artifact, with general-purpose tools (no committed script): (a) extract the contiguous **hard-fail zone** from the last line-anchored `### Sealed dispositions` heading to EOF (Sealed + Deferred + `### Latest pass detail` + `## Approval`), trailing-whitespace-normalized, from both the live original and the scratch draft, and `diff` — ANY difference is a hard fail → discard the scratch draft (the live artifact was never touched, so rollback is automatic) and revert; (b) `diff` the whole file (trailing-whitespace-normalized) — the body and `### Trajectory` may differ only by the named intended edit; revert if the body delta exceeds the approved scope OR drops/alters anything outside the intended change (the trigger is symmetric — omission disqualifies as much as excess); (c) on pass, snapshot the live artifact, promote the scratch draft over it, run structural validation on the live *directory* (the `validate_spec.py` / `validate_blueprint.py` directory-oriented validators cannot validate a lone scratch file), restore from the snapshot on a validation failure, then re-stamp. Structural validation alone is NOT sufficient — it checks section presence, not bullet/body content. The trivial-direct path above is unchanged, and an in-loop per-concern panel fix stays orchestrator-direct (the next pass of the same loop re-reviews it). This is the canonical delegation brief; the `panel-review.md` Substantive review-gate row and the `SKILL.md` backport sites cross-reference it.
...
2. **`--decline-pending`**: `python <script-path>/validate_spec.py specs/F<n>-<slug>/ --decline-pending` — an explicit, auditable USER decision to waive a genuinely-owed re-review.
...
**Unsatisfiable obligations — fresh reversed-order or legacy.** An UNSATISFIABLE obligation carries a genuine `upstream-panel` tag that sits on a pass at-or-below the recorded anchor, which the strictly-greater-than reconcile can never clear. It arises two ways: a **fresh reversed-order re-approval** (the anti-pattern below) or a **legacy** marker left by older tooling — the validator surfaces both with the same `UNSATISFIABLE-OBLIGATION:` diagnostic. Clear it with `--restore-anchor` — content-attested: it clears ONLY when the genuine tag is actually present on an archived row. `--decline-pending` is **refused** for such an obligation (it would falsely record the performed panel as skipped), and marker-cache hand-editing is never a close path. If `--restore-anchor` reports **nothing restorable** despite a tag you just stamped, the tag row may be an orphaned row (see below) or the obligation may have gone stale (Obligation survival, above) — resolve those first.

**Re-stamp FIRST, then panel — "review-then-approve" is the anti-pattern.** The doctrine order (re-stamp at step 2 BEFORE the panel at step 3e) opens the obligation before the fresh panel pass is archived, so the tag lands on a pass strictly ABOVE the anchor and the normal reconcile clears it with no further action. Doing it in the reverse "review-then-approve" order — archiving and tagging the panel pass BEFORE `--approve` — anchors the obligation at that same fresh pass, so the tag can never satisfy the strictly-greater-than reconcile and the obligation is UNSATISFIABLE. On a mixed prefix, run `--restore-anchor` BEFORE `--decline-pending` (restore-then-decline sequencing) so no UNSATISFIABLE entry survives into a decline.

**Orchestrator auto-restore (skill-driven flow).** When the orchestrator (the skill driving validate/approve/reconcile) detects a content-attested UNSATISFIABLE obligation during its own flow, it runs `--restore-anchor` ITSELF — scoped to the current directory's prefix, invoking the `--restore-anchor` CLI subcommand so it inherits the audit line — and surfaces a one-line note, rather than routing the human. This is safe because `--restore-anchor` is content-attested: it can never clear an un-performed panel, so auto-invocation is equivalent to a human running the same command. The fallback is trinary: auto-run in-skill; *missed* in-skill (an orchestrator that does not fire simply re-surfaces the `UNSATISFIABLE-OBLIGATION:` diagnostic on the next validate — graceful degradation to the manual path, still safe); or a human hand-running `--restore-anchor` outside the skill flow.
...
**Why source beats content here:** Claude is unreliable at judging the blast radius of its own edits — the very change that feels like "just a factual correction" is the one that silently invalidated a downstream acceptance criterion in the real failure this discipline exists to prevent. An author cannot see their own blind spot; the panel can. Leaning on source-classification removes the unreliable self-judgment from the loop.

**The flags are doctrine-classified facts, never convenience skips.** `--task-tick` asserts a specific, auditable fact — that this re-stamp is a Phase-4 task-completion tick (checkbox / status only), the one carve-out the doctrine grants. `--decline-pending` records that the USER decided to skip the panel for this obligation: a user decision the agent SURFACES and the user makes, never a self-serve shortcut the agent reaches for to dodge a panel it judged "unnecessary". Using either flag to wave through a substantive re-stamp IS the factual-edit shortcut wearing a flag — the exact failure mode the pending-review marker (`upstream-panel [0-9a-f]{8}`) and the changed-document reminder make auditable.
...
**Delegate the backport authorship.** Before making the upstream edit, the orchestrator delegates it to the **phase-correct analyst agent** for the artifact being edited (the "Drafted By" analyst in `references/workflow-overview.md` § Phase Summary, per tier — so this resolves to the analyst that originally drafted the upstream artifact). A trivial backport (typo, rename, single-sentence reword — no behavior change) stays orchestrator-direct per the existing triviality carve-out; a substantial one supplies the delegation brief and runs the scope-verify + content-preservation check before *Re-Approval After Edits* runs (both are defined in § "Revise the downstream" — Substantial, above).

**Mechanical gap (Phase 4).** A Phase-4 discovery reconciled only into `tasks.md` cannot propagate upward: `tasks.md` is terminal and has no downstream, so a fix absorbed only there never reaches `spec.md` / `design.md`. Always edit the affected upstream document — not `tasks.md` alone.
