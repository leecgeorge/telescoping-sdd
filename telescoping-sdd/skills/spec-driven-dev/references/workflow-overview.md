...
**1 — Gates are safe reset checkpoints.** Once a phase is approved, its decisions are captured in the artifact plus its content hash on disk. That is durable state, so the run can continue in a compacted or fresh session with **no state loss**. The safe checkpoints are the **Specify → Design → Tasks approvals** and the **Phase-4 / Implement completion gate** — Implement is the longest, most context-hungry phase, so resetting the moment it completes is the highest-value reset (confirm completion with the Phase-4 checklist below, not the Tasks-phase Approval checkbox).
...
**Phase-4 / Implement is special.** `tasks.md`'s `## Approval` checkbox was ticked back at the *Tasks* gate, *before* implementation began — so it is **NOT** a valid completion signal, and mid-implementation is **NOT** a clean reset point (`tasks.md` amends continuously and un-persisted implementation state is still open). Confirm Phase-4 completion with a **manual completion checklist**: all task checkboxes ticked; all Summary-table statuses `Done`; both the `## Implementation Deviations` and `## TDD Exceptions` ledgers resolved; and the final `tasks.md` re-stamp coherent (plain `--approve tasks`, or `--approve tasks --task-tick` for a pure-tick completion). This checklist is deliberately distinct from the `--completion-gate` CLI flag, which is **unrelated** — that flag only audits that `**Tests:**`-named tests exist (the R5 test-existence audit) and checks none of these four items. `--run-state` surfaces only the hash/obligation portion; confirm the ledger + Summary-status portion by reading `tasks.md` or running the full validator.
