...
- The **path** of the approved `specs/F<n>-<slug>/01_spec.md` as authoritative upstream context, with an instruction to **read it from disk in full** — do not paste or summarize its body into the prompt (the drafter reads the file(s) from disk, mirroring the panelist read-from-disk discipline)
- A clear instruction that the design must address every requirement and respect every boundary from the spec
- If `blueprint/PLAN.md` (bare or `NN_`-prefixed) exists and contains a `## Cross-Feature Contracts` section, give the agent that **file path** plus the `CFC-N` ids naming this feature (in their Participating features) as an orientation aid, and instruct it to **read the entire `## Cross-Feature Contracts` section from disk** — do not paste or summarize the section into the prompt; the ids highlight the relevant contracts, they do not restrict what the agent reads. The agent's CFC obligation (below) requires this input.
