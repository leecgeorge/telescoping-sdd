...
3. For each remaining concern, apply one of these dispositions:
   - **Addressed** — the fix is clear and stays within the current phase's scope. Apply the fix to the artifact and note what changed. **In-loop per-concern fixes are applied by the orchestrator directly**, not delegated to an analyst: analyst delegation applies at loop *re-entry* on a substantive change (cascade, gate-change re-draft, or backport), not per concern mid-loop. Why this is sound: an in-loop fix is re-reviewed by the very next pass of the same panel loop, so it gets the same stress-test an out-of-loop edit would otherwise lack — whereas a cascade/backport/gate-change edit gets no automatic re-pass, which is exactly why those are delegated. Observable boundary: "in-loop" means *inside an active multi-pass panel loop, disposing a concern this pass raised*; cascade and backport are never in-loop (no panel loop is running when they fire), so they cannot be reframed as in-loop per-concern fixes.
     ...
     <!-- SHARED-DOCTRINE:canonical-section-rule -->
     **Canonical-section rule.** Before dispositioning a concern `Addressed`, check whether the fact the fix would add is already stated elsewhere in the artifact. Each fact has **one home section**; other sections reference that home rather than re-explaining it. When it is already stated, add a cross-reference to the canonical section instead of restating — this stops accretion at the source rather than accreting then compressing. **Tie-break.** When two sections could host a fact, the home is the section whose single responsibility the fact defines (a field belongs in Data Models, not the Interface that uses it); if still tied, the earliest section in template/canonical order is the home and later sections cross-reference it.
     <!-- /SHARED-DOCTRINE:canonical-section-rule -->

     <!-- SHARED-DOCTRINE:under-doc-guardrail -->
     **Under-documentation guardrail.** The bias is *when in doubt, state it*. A cross-reference is permitted **only** when the fact is already fully and correctly stated at its home and the cross-reference drops nothing the concern requires; suppressing prose is the exception, not the default. **Worked example.** *Before (accretion):* Data Models: "`retry_count` is capped at 5"; Error Handling: "retries are capped at 5 attempts"; Interfaces: "the function retries up to 5 times." *After (canonical + cross-ref):* Data Models (home): "`retry_count`: int, max 5." Error Handling: "retries are capped — see Data Models `retry_count`." Interfaces: "retries up to the `retry_count` cap (see Data Models)." — one assertion, two cross-references.
     <!-- /SHARED-DOCTRINE:under-doc-guardrail -->

     <!-- SHARED-DOCTRINE:bake-resolution -->
     **Bake the resolution, not the debate.** When an `Addressed` fix resolves a debated concern, assert the flat resolution in the artifact body and keep the deliberation ("X but not Y", "NOT re-prosecuted here") in the ephemeral pass record (`### Latest pass detail` Notes, summarized into `### Trajectory` at archive). Durable rationale is carried only by a `Sealed`/`Accepted as risk` (`Defense:`) or `Deferred` (`Routed because:`) disposition — never by prose in the artifact body. No new artifact, no hash change.
     <!-- /SHARED-DOCTRINE:bake-resolution -->
...
> **Not situational — loads at every convergence.** One behaviour is the exception to this list: the terminal compression check (`panel-review-convergence.md § Terminal Compression Check`) is **not** trigger-gated. Unlike the five situational behaviours here, a NORMAL pass *does* reach it — it runs once at every convergence, on any exit path, immediately before validation. It is homed in the convergence sub-reference only for co-location with the exit cross-check it is contrasted against.
